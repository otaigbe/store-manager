...
function createBill() {
  ...
  headrow.setAttribute("id","salestableheadrow");
	...
	const productID = document.createTextNode("ProductId");
  const productDescription = document.createTextNode("Product Descrition");
  var priceheading = document.createTextNode("Unit Price");
  const quantityheading = document.createTextNode("Qty Purchased");
  ...
  const selectedColumn = cart.rows[0].cells[3];

  for (let i =0; i < cart.rows.length; i++) {
    const cell = cart.rows[i].cells[3];
  ...
  for (let i =0; i < cart.rows.length; i++) {
    const cell = cart.rows[i].cells[6];
    cell.parentElement.removeChild(cell);
    // console.log();
  ...
  for (let x = 0; x < rows_to_be_selected.length; x++) {
...
function convertTableToJson(table){
  let cols= []; 
    let headerlist = table.querySelectorAll("thead tr th");
...
let tableobj =  [];
let rowlist = table.querySelectorAll("tbody tr");
//console.log(rowlist);
for (let j=0; j<rowlist.length; j++){
  let tdlist = rowlist[j].querySelectorAll("td");
  let row = {};
  //console.log(rowlist[j].textContent.toString().trim());
   for (let k=0; k < tdlist.length; k++){
     let text = tdlist[k].textContent.toString().trim();
     let rowName  =cols[k];
     row[rowName] = text;
     console.log(text)
   }
...
function calculateTotalAmount(row) {
  let individualAmount = row.querySelector(".amount").value;
  let number = Number(individualAmount)
  //console.log(number);
  return number;
}
